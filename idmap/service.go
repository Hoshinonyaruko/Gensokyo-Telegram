...
func ReadConfig(sectionName, keyName string) (string, error) {
	// // 确保配置项存在，如果不存在则创建并设置初始值
	// if err := ensureConfig(db, sectionName, keyName, "0"); err != nil {
	// 	return "", err
	// }
	var result string
	err := db.View(func(tx *bolt.Tx) error {
...
// ensureConfig 确保指定的section和key存在，如果不存在，则创建它们并写入初始值
func ensureConfig(db *bolt.DB, sectionName, keyName, defaultValue string) error {
	return db.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists([]byte(ConfigBucket))
		if err != nil {
		...
		if v := b.Get([]byte(key)); v == nil {
			// Key不存在，写入默认值
			if err := b.Put([]byte(key), []byte(defaultValue)); err != nil {
				return fmt.Errorf("failed to set default value for key '%s' in section '%s': %v", keyName, sectionName, err)
			}
		}
		return nil
	})
