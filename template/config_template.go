...
settings:
  ...
  botToken: "12345"                                    # 在telegram软件内 botfather获取 例子 111260888:BBFf5h2cSDoUZctABCD-spo0b2dG4gyEGEG
  webHookPath : ""                                     # 你的Webhook 地址 请放通端口,请固定使用443端口 例子 https://www.example.com:443/
  useNgrok : false                                     # 使用Ngrok临时域名作为webhook地址,结合customcert=true可无需域名无需证书使用webhook
  ngrokKey : ""                                        # https://dashboard.ngrok.com/get-started/setup/windows 获取 例子: 2dht5Di4gbBVdB4xpPlFZMhuBSQ_59XGGowvtYC1xyABUjCdt
  httpGetMsg : false                                   # 如果你不打算使用webhook,那么使用http轮询获取api(强烈建议使用webhook)
  getMsgTimeOut : 3600                                 # 使用http获取信息超时时间,即多少秒以后没有信息就断开连接.默认一小时
  highway : true                                       # 将图片url直接上传到telegram 服务器带宽低情况比发图片url更快 请自行感受速度决定是否开启
  sendDirectResponse : false                           # 是否在webhook返回时直接发送信息(在不同服务器条件,速度或更快或更慢都有可能) 请自行感受速度决定是否开启
  ...
  port: "443"                                        # idmaps和图床对外开放的端口号 默认443因为tg要求强制https
  ...
  default_content : ["",""]         #Telegram机器人是自由触发,当后端没有返回数据时,用户易流失,可设置多个随机的兜底回复,提示帮助指令,文档等.如果发不出,请减小timeOut的值
  ...
  customcert : false                  #telegram默认支持自签名证书,省去配置ssl烦恼.进阶用户也可使用nginx反代部署机器人.
  crt : ""                           #证书路径 从你的域名服务商或云服务商申请签发SSL证书(tg要求SSL)  实例 cert.pem 可以是本地路径/需要双写
  key : ""                           #密钥路径 pem格式 示例: "key.pem" 放在exe同目录下
